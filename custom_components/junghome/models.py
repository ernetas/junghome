...
class Scene(TypedDict):
    ...
    ``id`` is ``"id"`` + the mesh scene number in hex (``id0001`` <->
    ``value`` ``"0001"``), so it is in fact stabler than a device id — but the
    scene platform anchors identity on the ``label`` (the unique_id scheme
    existing installs carry) and re-resolves ``id`` from the coordinator's
    scene list at activation time.
    """
