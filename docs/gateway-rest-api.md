...
| GET  | `/project/cdb`, `/project/junghome` ; PATCH `/project` | Project / mesh DB export *(fw 1.5.0+)*. **`/project/cdb` returns the Bluetooth Mesh CDB including NetKey, AppKeys and every device key**; `/project/junghome` is the app's `ExportDto` (node UUID / MAC / unicast / locations per device — the hardware identity the `functions` payload lacks). |
