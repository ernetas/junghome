...
- **`GET /devices/?verbose=true` (deprecated/experimental in the OpenAPI,
  ...
  sensor (Wh, `TOTAL_INCREASING`, `sensor.<socket>_total_energy`), each
  device page's `sw_version` and the per-device duplicate-suppression
  exemption (`button_reports_each_tap_once`: revision known AND < 2.2.0) —
  both through `coordinator.software_revision_for`, which resolves a null
